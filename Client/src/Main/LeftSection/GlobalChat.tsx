...
const GlobalChat = ({ setCurrentopenchatid }: any) => {
  ...
  const [ValuesOfLatestMessage, setValuesOfLatestMessage] = useState<
    any | undefined
  >();
  ...
  useEffect(() => {
    ...
    async function fetchlatestmessage() {
      ...
      if (data && data?.length > 0) {
        getname(data[0].Sender)
          .then((name) => setname(name))
          .catch((e) => {
            console.log("couldn't get name " + e);
          });
        ...
        console.log(
          error +
            " error occured while getting latest message in message, please ignore if global chat has no messages"
        );
        setValuesOfLatestMessage({});
      }
    }
